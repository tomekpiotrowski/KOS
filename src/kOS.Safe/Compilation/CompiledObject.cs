...
namespace kOS.Safe.Compilation
{
    ...
    public static class CompiledObject
    {
        ...
        /// <returns></returns>
        public static List<CodePart> UnPack(GlobalPath filePath, string prefix, byte[] content)
        {
        ...
        private static void PostReadProcessing(IEnumerable<CodePart> program, GlobalPath filePath, string prefix, DebugLineMap lineMap)
        {
            ...
            foreach (CodePart part in program)
            {
                ...
                foreach (List<Opcode> codeList in sections)
                {
                    foreach (Opcode op in codeList)
                    {
                        ...
                        op.SourcePath = (op.SourcePath == null || op.SourcePath == GlobalPath.EMPTY) ? filePath : GlobalPath.EMPTY;
                    }
                }
            }
        }
