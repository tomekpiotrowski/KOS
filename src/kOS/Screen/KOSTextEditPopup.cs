...
namespace kOS.Screen
{
    ...
    public class KOSTextEditPopup : KOSManagedWindow
    {
        ...
        {
            VolumeFile file = volume.GetOrCreateFile(path);
            ...
            isDirty = false;
            term.Print("[Saved changes to " + path + "]");
        }
        ...
        protected void InvokeDirtySaveExitDialog()
        {
            ...
            dialog.Invoke( this, "\"" + path.Name + "\" has been edited.  Save it before exiting?", choices, actions );
        }
        ...
        protected void InvokeDirtySaveLoadDialog()
        {
            ...
            dialog.Invoke( this, "\"" + path.Name + "\" has been edited.  Save before loading \"" + loadingPath.Name + "\"?", choices, actions );
        }
        ...
        protected void InvokeReloadConfirmDialog()
        {
            ...
            dialog.Invoke( this, "\"" + path.Name + "\" has been edited.  Throw away changes and reload?", choices, actions );
        }
        ...
        protected static void DelegateLoadContents(KOSTextEditPopup me)
        {
            ...
            me.path = me.loadingPath;
            VolumeItem item = me.volume.Get(me.path);

            if (item == null) {
                me.term.Print ("[New File]");
                me.contents = "";
            }
            else if (item is VolumeDirectory)
            {
                throw new KOSException("Can't read contents of a directory");
            }
            else
            {
                VolumeFile file = item as VolumeFile;
                me.contents = Encoding.UTF8.GetString(file.Read());
            }
        ...
        protected string BuildTitle()
        {
            if (volume.Name.Length > 0)                
                return path.Name + " on " + volume.Name;
            return path.Name + " on local volume";  // Don't know which number because no link to VolumeManager from this class.
        }
    }
}
